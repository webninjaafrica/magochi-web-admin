<?php
// ================================================================
// MAGOCHI HOST ADMIN v7.0 – COMPLETE CPANEL-STYLE PANEL
// All features: Domains, Databases, FTP, Cron (API), File Manager,
// Admin Panel, User Switch, Disk Allocation, Theme, Logs,
// TEXT EDITOR, UNZIP, HTACCESS GENERATOR, FTP CLIENT,
// API MANAGEMENT, WORDPRESS INSTALLER, CPANEL LAYOUT
// Developed by: Kelvin Magochi | +254718265708 | admin@webninjaafrica.com
// ================================================================
...
$attempts = [
    ['host' => 'localhost', 'user' => $db_user, 'pass' => $db_pass, 'port' => 3306],
    ['host' => 'localhost', 'user' => $db_user, 'pass' => '', 'port' => 3306],
    ['host' => '127.0.0.1', 'user' => $db_user, 'pass' => $db_pass, 'port' => 3306],
    ['host' => '127.0.0.1', 'user' => $db_user, 'pass' => '', 'port' => 3306],
];
...
if (!$connected) {
    die("<h2>MySQL Connection Error</h2><p>Start MySQL: <code>mysqld_safe &amp;</code></p><p>Contact: $phone</p>");
}
...
if ($check->num_rows == 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO users (username, password, full_name, email, role, status, disk_limit) VALUES (?, ?, ?, ?, 'admin', 'active', 9999)");
    $stmt->bind_param("ssss", 'admin', $hashed, $developer, $email);
    $stmt->execute();
    $stmt->close();
}

$root = $_SERVER['DOCUMENT_ROOT'];
$dirs = [$root . '/domains', $root . '/uploads', $root . '/backups', $root . '/temp'];
foreach ($dirs as $d) {
...
function getDirectoryContents($dir, $user_id = null, $is_admin = false) {
    $items=[]; 
    if (is_dir($dir)){ 
        $scan=scandir($dir); 
        foreach($scan as $item){ 
            if($item!='.' && $item!='..'){ 
                ...
                $items[]=[
                    ...
                    'editable'=>is_file($path)&&in_array(pathinfo($path,PATHINFO_EXTENSION),['php','html','htm','css','js','txt','sql','json','xml','yml','ini','htaccess','md'])
                ]; 
            } 
        } 
...
function formatSize($bytes){ if($bytes==0)return'0 B';$units=['B','KB','MB','GB','TB'];$i=floor(log($bytes,1024));return round($bytes/pow(1024,$i),2).' '.$units[$i];}
function getFileIcon($path){ if(is_dir($path))return'fa-folder'; $ext=pathinfo($path,PATHINFO_EXTENSION); $icons=['php'=>'fa-php','html'=>'fa-html5','css'=>'fa-css3','js'=>'fa-js','txt'=>'fa-file-alt','log'=>'fa-file-alt','jpg'=>'fa-image','png'=>'fa-image','gif'=>'fa-image','svg'=>'fa-image','pdf'=>'fa-file-pdf','doc'=>'fa-file-word','docx'=>'fa-file-word','xls'=>'fa-file-excel','xlsx'=>'fa-file-excel','zip'=>'fa-file-archive','tar'=>'fa-file-archive','gz'=>'fa-file-archive','sql'=>'fa-database','json'=>'fa-code','xml'=>'fa-code','yml'=>'fa-code','ini'=>'fa-cog','htaccess'=>'fa-lock','md'=>'fa-file-alt']; return $icons[strtolower($ext)]??'fa-file';}
function deleteFolder($path){ if(!is_dir($path))return false; $items=scandir($path); foreach($items as $item){ if($item!='.'&&$item!='..'){ $full=$path.'/'.$item; if(is_dir($full)) deleteFolder($full); else unlink($full); } } return rmdir($path); }
function getServerInfo(){ $info=[]; $info['server_software']=$_SERVER['SERVER_SOFTWARE']??'Unknown'; $info['php_version']=phpversion(); $info['server_name']=$_SERVER['SERVER_NAME']??'localhost'; $info['document_root']=$_SERVER['DOCUMENT_ROOT']??''; $info['server_ip']=$_SERVER['SERVER_ADDR']??'127.0.0.1'; $info['upload_max_filesize']=ini_get('upload_max_filesize'); $info['post_max_size']=ini_get('post_max_size'); $info['memory_limit']=ini_get('memory_limit'); $info['max_execution_time']=ini_get('max_execution_time').'s'; $info['mysql_version']=$GLOBALS['conn']->server_info; return $info; }
function getDatabaseList(){ global $conn; $dbs=[]; $res=$conn->query("SHOW DATABASES"); while($row=$res->fetch_assoc()) $dbs[]=$row['Database']; return $dbs; }
function backupDatabase($dbname){ global $conn,$root; $filepath=$root.'/backups/backup_'.$dbname.'_'.date('Y-m-d_H-i-s').'.sql'; $tables=[];$res=$conn->query("SHOW TABLES FROM `$dbname`");while($row=$res->fetch_row())$tables[]=$row[0]; $output="-- Database: $dbname\n-- Generated: ".date('Y-m-d H:i:s')."\n\n"; foreach($tables as $t){ $res=$conn->query("SELECT * FROM `$dbname`.`$t`"); $create=$conn->query("SHOW CREATE TABLE `$dbname`.`$t`"); $create_row=$create->fetch_row(); $output.="DROP TABLE IF EXISTS `$t`;\n".$create_row[1].";\n\n"; if($res->num_rows>0){ while($row=$res->fetch_row()){ $esc=array_map(function($v)use($conn){return $conn->real_escape_string($v);},$row); $output.="INSERT INTO `$t` VALUES ('".implode("', '",$esc)."');\n"; } $output.="\n"; } } file_put_contents($filepath,$output); return $filepath; }
function calculateFolderSize($dir){ $size=0; if(is_dir($dir)){ $files=scandir($dir); foreach($files as $f){ if($f!='.'&&$f!='..'){ $path=$dir.'/'.$f; if(is_dir($path))$size+=calculateFolderSize($path); else $size+=filesize($path); } } } return $size; }
...
function unzipFile($zipPath, $extractTo) {
    ...
    if ($zip->extractTo($extractTo)) { $zip->close(); return array('success' => true, 'files' => $zip->numFiles); }
    $zip->close();
...
function getUserID() { return $_SESSION['user_id'] ?? 0; }
function getUserRole() { return $_SESSION['role'] ?? 'user'; }
...
if (isset($_POST['register'])) {
    ...
    if (empty($username) || empty($password) || empty($email) || empty($full_name)) {
        ...
        if ($check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, password, email, full_name, role, status) VALUES (?, ?, ?, ?, 'user', 'active')");
...
$currentPath = isset($_GET['path']) ? $_GET['path'] : $root;

// Security: restrict path for non-admin - ONLY SHOW USER'S OWN FILES
$allowed_paths = [$root];
if (!$is_admin) {
    $domains = getUserDomains($user_id);
    while ($d = $domains->fetch_assoc()) {
        $allowed_paths[] = $domains_root.'/'.$d['folder'];
    }
    $domains->data_seek(0);
}
$ok = false;
foreach ($allowed_paths as $p) {
    if (strpos($currentPath, $p) === 0) { $ok = true; break; }
}
if (!$ok) $currentPath = $root;

// ================================================================
// API ENDPOINT FOR CRON
// ================================================================
if (isset($_GET['api']) && $_GET['api'] == 'cron') {
    header('Content-Type: application/json');
    if (!isset($_GET['key']) || $_GET['key'] !== $api_secret) {
        echo json_encode(['status' => 'error', 'message' => 'Invalid API key']);
        exit;
    }
    $action = $_GET['action'] ?? 'run_all';
    if ($action == 'run_all') {
        $result = $conn->query("SELECT * FROM cron_jobs WHERE enabled = 1");
        $output = [];
        while ($cron = $result->fetch_assoc()) {
            ...
            curl_close($ch);
            $conn->query("UPDATE cron_jobs SET last_run = NOW() WHERE id = " . $cron['id']);
            $output[] = ['name' => $cron['name'], 'url' => $cron['url'], 'status' => $http_code];
        }
        echo json_encode(['status' => 'success', 'jobs_run' => count($output), 'results' => $output]);
        exit;
    }
}
...
if ($is_admin && isset($_GET['switch_user'])) {
    ...
    } else { $message = "User not found!"; $msg_type = 'error'; }
}
...
if ($is_admin && isset($_POST['admin_create_user'])) {
    ...
    if (!empty($username) && !empty($password) && !empty($email) && !empty($fullname)) {
        if ($conn->query("SELECT id FROM users WHERE username='$username'")->num_rows > 0) {
            $message = "Username already exists!"; $msg_type='error';
        } else {
            $hashed = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $conn->prepare("INSERT INTO users (username, password, email, full_name, role, status, disk_limit) VALUES (?, ?, ?, ?, ?, 'active', ?)");
            $stmt->bind_param("sssssi", $username, $hashed, $email, $fullname, $role, $disk_limit);
            if ($stmt->execute()) {
    ...
    } else { $message = "All fields required!"; $msg_type='error'; }
}
...
if ($is_admin && isset($_POST['admin_create_domain'])) {
    ...
    $folder = trim($_POST['admin_folder']) ?: str_replace(['.', ' '], '_', $domain);
    if (!empty($domain) && $uid>0) {
        if ($conn->query("SELECT id FROM users WHERE id=$uid")->num_rows == 0) {
            ...
            if ($conn->query("SELECT id FROM domains WHERE domain='$domain'")->num_rows > 0) {
                $message = "Domain already exists!"; $msg_type='error';
            } else {
                $path = $domains_root.'/'.$folder;
                if (!is_dir($path)) mkdir($path, 0755, true);
                file_put_contents($path.'/index.html', "<h1>Welcome to $domain</h1><p>Hosted on Magochi Host</p>");
    ...
    } else { $message = "Domain and User ID required!"; $msg_type='error'; }
}
    ...
    if (!empty($dbname) && $uid>0) {
        if ($conn->query("SELECT id FROM users WHERE id=$uid")->num_rows == 0) {
            ...
            if ($conn->query("CREATE DATABASE IF NOT EXISTS `$dbname`")) {
                try { $conn->query("GRANT ALL PRIVILEGES ON `$dbname`.* TO '$dbuser'@'localhost' IDENTIFIED BY '$dbpass'"); $conn->query("FLUSH PRIVILEGES"); } catch(Exception $e){}
                ...
                $stmt->close();
            } else { $message = "Error creating database!"; $msg_type='error'; }
        }
    } else { $message = "Database name and User ID required!"; $msg_type='error'; }
}
    ...
    if (!empty($username) && !empty($password) && $uid>0) {
        if ($conn->query("SELECT id FROM users WHERE id=$uid")->num_rows == 0) {
            ...
            if ($conn->query("SELECT id FROM ftp_users WHERE username='$username'")->num_rows > 0) {
                $message = "FTP username already exists!"; $msg_type='error';
            } else {
                $hashed = password_hash($password, PASSWORD_DEFAULT);
...
if ($is_admin && isset($_POST['admin_create_cron'])) {
    ...
    if (!empty($name) && !empty($url) && !empty($schedule) && $uid>0) {
        if (!filter_var($url, FILTER_VALIDATE_URL)) { $message = "Invalid URL!"; $msg_type='error'; }
        else {
            if ($conn->query("SELECT id FROM users WHERE id=$uid")->num_rows == 0) {
                $message = "User not found!"; $msg_type='error';
            } else {
...
// ----- ADMIN DELETE USER, DOMAIN, DATABASE, etc. -----
if ($is_admin) {
...
if (!$is_admin && isset($_POST['create_domain'])) {
    ...
    $folder = trim($_POST['folder']) ?: str_replace(['.', ' '], '_', $domain);
    $path = $domains_root.'/'.$folder;
    $count = $conn->query("SELECT COUNT(*) as c FROM domains WHERE user_id=$user_id")->fetch_assoc()['c'];
    if ($count >= 5) { $message = "Max 5 domains!"; $msg_type='error'; }
    elseif (!empty($domain)) {
        if ($conn->query("SELECT id FROM domains WHERE domain='$domain'")->num_rows > 0) {
            $message = "Domain already exists!"; $msg_type='error';
        } else {
...
if (!$is_admin && isset($_POST['create_db'])) {
    ...
    if (!empty($dbname)) {
        ...
        if ($count >= 3) { $message = "Max 3 databases!"; $msg_type='error'; }
        else {
            if ($conn->query("CREATE DATABASE IF NOT EXISTS `$dbname`")) {
                try { $conn->query("GRANT ALL PRIVILEGES ON `$dbname`.* TO '$dbuser'@'localhost' IDENTIFIED BY '$dbpass'"); $conn->query("FLUSH PRIVILEGES"); } catch(Exception $e){}
                $stmt = $conn->prepare("INSERT INTO user_databases (user_id, db_name, db_user, db_pass) VALUES (?, ?, ?, ?)");
...
if (!$is_admin && isset($_POST['create_ftp'])) {
    ...
    $home = trim($_POST['ftp_home']) ?: $root;
    if (!empty($username) && !empty($password)) {
        if ($conn->query("SELECT id FROM ftp_users WHERE username='$username'")->num_rows > 0) {
            $message = "FTP username exists!"; $msg_type='error';
        } else {
            $hashed = password_hash($password, PASSWORD_DEFAULT);
            $stmt = $conn->prepare("INSERT INTO ftp_users (user_id, username, password, home_dir) VALUES (?, ?, ?, ?)");
...
if (isset($_POST['install_wordpress'])) {
    ...
    $admin_pass = trim($_POST['wp_admin_pass']);
    ...
    if (!empty($domain_id) && !empty($admin_user) && !empty($admin_pass) && !empty($admin_email)) {
        ...
        if ($domain_info) {
            ...
            // Download WordPress
            $temp_zip = $root . '/temp/wordpress.zip';
            ...
            if ($http_code == 200 && file_put_contents($temp_zip, $data)) {
                // Extract WordPress
                $zip = new ZipArchive();
                if ($zip->open($temp_zip) === true) {
                    ...
                    $db_info = $conn->query("SELECT * FROM user_databases WHERE user_id=$user_id LIMIT 1")->fetch_assoc();
                    if ($db_info) {
                        $wp_config = "<?php\n";
                        $wp_config .= "define('DB_NAME', '{$db_info['db_name']}');\n";
                        $wp_config .= "define('DB_USER', '{$db_info['db_user']}');\n";
                        $wp_config .= "define('DB_PASSWORD', '{$db_info['db_pass']}');\n";
                    ...
                    $stmt->bind_param("iisssss", $user_id, $domain_id, $path, $wp_version, $admin_user, $admin_pass, $admin_email);
                    $stmt->execute();
                    $stmt->close();
                    
                    $message = "WordPress installed successfully!<br>Admin: $admin_user<br>Pass: $admin_pass<br>Email: $admin_email";
                    ...
                    $message = "Failed to extract WordPress!";
                    $msg_type = 'error';
                }
            } else {
                $message = "Failed to download WordPress!";
                $msg_type = 'error';
            }
        } else {
...
if (isset($_FILES['uploaded_file'])) {
    ...
    if (!$allowed) {
        $domains = getUserDomains($user_id);
        while ($d = $domains->fetch_assoc()) {
            if (strpos($target_dir, $domains_root.'/'.$d['folder']) === 0) { $allowed = true; break; }
        }
        $domains->data_seek(0);
    }
    if ($allowed) {
    ...
    $conn->query("INSERT INTO activity_logs (user_id, action, details, ip) VALUES ($user_id, 'Upload File', 'Uploaded '.{$file['name']}, '{$_SERVER['REMOTE_ADDR']}')");
        } else { $message = "Upload failed!"; $msg_type='error'; }
    } else { $message = "Permission denied!"; $msg_type='error'; }
}
...
if (isset($_POST['create_file'])) {
    ...
    if (!$allowed) {
        $domains = getUserDomains($user_id);
        while ($d = $domains->fetch_assoc()) {
            if (strpos($currentPath, $domains_root.'/'.$d['folder']) === 0) { $allowed = true; break; }
        }
        $domains->data_seek(0);
    }
    if ($allowed && !empty($filename)) {
        ...
        } else { $message = "Failed to create file!"; $msg_type='error'; }
    } else { $message = "Permission denied or empty filename!"; $msg_type='error'; }
}
...
if (isset($_POST['create_folder'])) {
    ...
    if ($allowed && !empty($foldername)) {
        ...
        } else { $message = "Failed to create folder!"; $msg_type='error'; }
    } else { $message = "Permission denied!"; $msg_type='error'; }
}
...
if (isset($_GET['delete_file'])) {
    ...
    if (!$allowed) {
        $domains = getUserDomains($user_id);
        while ($d = $domains->fetch_assoc()) {
            if (strpos($file, $domains_root.'/'.$d['folder']) === 0) { $allowed = true; break; }
        }
        $domains->data_seek(0);
    }
    if ($allowed && file_exists($file) && is_file($file)) {
        ...
        } else { $message = "Delete failed!"; $msg_type='error'; }
    } else { $message = "Permission denied or file not found!"; $msg_type='error'; }
}
...
if (isset($_GET['delete_folder'])) {
    ...
    if (!$allowed) {
        $domains = getUserDomains($user_id);
        while ($d = $domains->fetch_assoc()) {
            if (strpos($folder, $domains_root.'/'.$d['folder']) === 0) { $allowed = true; break; }
        }
        $domains->data_seek(0);
    }
    if ($allowed && is_dir($folder) && $folder != $root && $folder != $domains_root) {
...
if (isset($_POST['save_file'])) {
    ...
    if (!$allowed) {
        $domains = getUserDomains($user_id);
        while ($d = $domains->fetch_assoc()) {
            if (strpos($filepath, $domains_root.'/'.$d['folder']) === 0) { $allowed = true; break; }
        }
        $domains->data_seek(0);
    }
    if ($allowed && file_exists($filepath) && is_file($filepath)) {
        ...
        } else { $message = "Save failed!"; $msg_type='error'; }
    } else { $message = "Permission denied!"; $msg_type='error'; }
}
...
if (isset($_POST['save_theme'])) {
    $_SESSION['theme'] = [
        'primary' => $_POST['primary_color'] ?? '#6C63FF',
...
$files = getDirectoryContents($currentPath, $user_id, $is_admin);
$logs = getActivityLogs($is_admin ? null : $user_id);

// Calculate disk usage for current user
$disk_used = 0;
...
$disk_limit = $user_data['disk_limit'] ?? 100;
...
// Theme
$primary = $_SESSION['theme']['primary'] ?? '#6C63FF';
$dark = $_SESSION['theme']['dark_mode'] ?? false;
...
if (!isset($_SESSION['user_id']) && !isset($_POST['login']) && !isset($_POST['register'])) {
    ...
    <div class="footer">Developed by <strong>Kelvin Magochi</strong> | 📞 <?php echo htmlspecialchars($phone); ?></div></div>
...
// ================================================================
// START HTML OUTPUT - CPANEL STYLE WITH RIGHT SIDEBAR
// ================================================================
?><!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Magochi Host Admin – <?php echo htmlspecialchars(ucfirst($action)); ?></title>
...
<div class="sidebar" id="sidebar">
    ...
    <ul class="sidebar-nav">
        <li><a href="?action=dashboard" class="<?php echo $action=='dashboard'?'active':''; ?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
        <?php if (!$is_admin): ?>
        <li><a href="?action=domains" class="<?php echo $action=='domains'?'active':''; ?>"><i class="fas fa-globe"></i> Domains</a></li>
        <li><a href="?action=databases" class="<?php echo $action=='databases'?'active':''; ?>"><i class="fas fa-database"></i> Databases</a></li>
        <li><a href="?action=ftp" class="<?php echo $action=='ftp'?'active':''; ?>"><i class="fas fa-user-lock"></i> FTP Users</a></li>
        <li><a href="?action=files" class="<?php echo $action=='files'?'active':''; ?>"><i class="fas fa-folder"></i> File Manager</a></li>
        <li><a href="?action=apps" class="<?php echo $action=='apps'?'active':''; ?>"><i class="fas fa-th"></i> Apps</a></li>
        <?php else: ?>
        <li><a href="?action=admin_users" class="<?php echo $action=='admin_users'?'active':''; ?>"><i class="fas fa-users-cog"></i> Admin Users</a></li>
        <li><a href="?action=admin_domains" class="<?php echo $action=='admin_domains'?'active':''; ?>"><i class="fas fa-globe-americas"></i> Admin Domains</a></li>
        <li><a href="?action=admin_databases" class="<?php echo $action=='admin_databases'?'active':''; ?>"><i class="fas fa-database"></i> Admin Databases</a></li>
        <li><a href="?action=admin_ftp" class="<?php echo $action=='admin_ftp'?'active':''; ?>"><i class="fas fa-user-lock"></i> Admin FTP</a></li>
        <li><a href="?action=admin_cron" class="<?php echo $action=='admin_cron'?'active':''; ?>"><i class="fas fa-clock"></i> Admin Cron</a></li>
        <?php endif; ?>
        <li><a href="?action=services" class="<?php echo $action=='services'?'active':''; ?>"><i class="fas fa-link"></i> Services</a></li>
        <li><a href="?action=cron" class="<?php echo $action=='cron'?'active':''; ?>"><i class="fas fa-clock"></i> Cron Jobs</a></li>
        <li><a href="?action=backups" class="<?php echo $action=='backups'?'active':''; ?>"><i class="fas fa-archive"></i> Backups</a></li>
        <li><a href="?action=logs" class="<?php echo $action=='logs'?'active':''; ?>"><i class="fas fa-history"></i> Activity Logs</a></li>
        <li><a href="?action=api" class="<?php echo $action=='api'?'active':''; ?>"><i class="fas fa-key"></i> API Management</a></li>
        <?php if ($is_admin): ?><li class="divider"></li>
        <li><a href="?action=users" class="<?php echo $action=='users'?'active':''; ?>"><i class="fas fa-users"></i> All Users</a></li>
        <li><a href="?action=server" class="<?php echo $action=='server'?'active':''; ?>"><i class="fas fa-server"></i> Server</a></li><?php endif; ?>
        <li><a href="?action=settings" class="<?php echo $action=='settings'?'active':''; ?>"><i class="fas fa-cog"></i> Settings</a></li>
...
<div class="main">
    ...
    <div class="card">
        ...
        <div class="cpanel-grid">
            ...
            <?php else: ?>
            <a href="?action=admin_users" class="cpanel-item"><span class="icon"><i class="fas fa-users-cog"></i></span><span class="label">Users</span><span class="desc">Manage users</span></a>
            <a href="?action=admin_domains" class="cpanel-item"><span class="icon"><i class="fas fa-globe-americas"></i></span><span class="label">Domains</span><span class="desc">Manage domains</span></a>
            <a href="?action=admin_databases" class="cpanel-item"><span class="icon"><i class="fas fa-database"></i></span><span class="label">Databases</span><span class="desc">Manage databases</span></a>
            <a href="?action=admin_ftp" class="cpanel-item"><span class="icon"><i class="fas fa-user-lock"></i></span><span class="label">FTP</span><span class="desc">Manage FTP users</span></a>
            <a href="?action=admin_cron" class="cpanel-item"><span class="icon"><i class="fas fa-clock"></i></span><span class="label">Cron</span><span class="desc">Manage cron jobs</span></a>
            <?php endif; ?>
    ...
    <!-- ================================================================ -->
    <?php if ($action == 'api'): ?>
    <div class="card"><div class="card-header"><h3><i class="fas fa-key"></i> API Management</h3></div>
    <div style="background:var(--bg); padding:15px; border-radius:10px; margin-bottom:15px;">
    ...
    <!-- ================================================================ -->
    <?php if ($action == 'files'): ?>
    <div class="card"><div class="card-header"><h3><i class="fas fa-folder"></i> File Manager</h3><span style="font-size:11px; color:#999;"><?php echo htmlspecialchars($currentPath); ?></span></div>
    ...
    <!-- Breadcrumb -->
    <div class="breadcrumb"><?php $parts = explode('/', trim(str_replace($root, '', $currentPath), '/')); $p = $root; echo '<a href="?action=files&path='.urlencode($p).'"><i class="fas fa-home"></i> Root</a>'; foreach ($parts as $part) { if (!empty($part)) { $p .= '/'.$part; echo ' / <a href="?action=files&path='.urlencode($p).'">'.htmlspecialchars($part).'</a>'; } } ?><span style="margin-left:auto; font-size:11px; color:#999;"><?php echo count($files); ?> items</span></div>

    <!-- File List -->
    <div><?php if ($currentPath != $root): ?><div class="file-item"><div class="name"><i class="fas fa-arrow-up"></i> <a href="?action=files&path=<?php echo urlencode(dirname($currentPath)); ?>"><strong>..</strong></a></div></div><?php endif; ?>
    ...
    <!-- View File Modal -->
    <?php if (isset($_GET['view_file'])): $view_file = $_GET['view_file']; $view_content = file_exists($view_file) ? file_get_contents($view_file) : ''; $ext = pathinfo($view_file, PATHINFO_EXTENSION); $is_image = in_array(strtolower($ext), ['jpg','jpeg','png','gif','svg','webp','bmp']); ?>
    ...
    <?php if (in_array(strtolower($ext), ['php','html','htm','css','js','txt','sql','json','xml','yml','ini','htaccess','md'])): ?>
    ...
    <div style="background:var(--bg); padding:15px; border-radius:10px;"><p><strong>Magochi Host Admin</strong> v<?php echo htmlspecialchars($version); ?></p><p>Developed by <strong><?php echo htmlspecialchars($developer); ?></strong></p><p>📞 <?php echo htmlspecialchars($phone); ?></p><p>✉️ <?php echo htmlspecialchars($email); ?></p><p style="font-size:11px; color:#999; margin-top:8px;"><i class="fas fa-copyright"></i> 2026 Magochi Host - All Rights Reserved</p></div></div>
    <?php endif; ?>

    <div class="footer"><p><strong>Magochi Host Admin</strong> v<?php echo htmlspecialchars($version); ?> | Developed by <strong><?php echo htmlspecialchars($developer); ?></strong> | 📞 <?php echo htmlspecialchars($phone); ?> | ✉️ <?php echo htmlspecialchars($email); ?></p></div>
</div>
